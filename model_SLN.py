...
from nets_tensorflow import resnet_v1
    ...
    with tf.variable_scope('feature_fusion', values=[end_points.values]):
        ...
        with slim.arg_scope([slim.conv2d],
            ...
            for i in range(4):
                ...
                if i <= 2:
                    g[i] = unpool(h[i])
                    g[i] = slim.conv2d(g[i],num_outputs[i],3)
            ...
            for scale in range(4):
                ...
                # For the up-sampling operator
                if scale==3:
                    continue
                m_g = slim.conv2d(g[scale],32,3)
                score_map_g = slim.conv2d(m_g,1,1,activation_fn=tf.nn.sigmoid,normalizer_fn=None)
                geo_map_g = slim.conv2d(m_g, 4, 1, activation_fn=tf.nn.sigmoid, normalizer_fn=None) * FLAGS.text_scale
                angle_map_g = (slim.conv2d(m_g, 1, 1, activation_fn=tf.nn.sigmoid,normalizer_fn=None) - 0.5) * np.pi / 2  # angle is between [-45, 45]
                geo_g = tf.concat([geo_map_g, angle_map_g], axis=-1)
                F_score_g.append(score_map_g)
                F_geometry_g.append(geo_g)


    return F_score, F_geometry, F_score_g, F_geometry_g
